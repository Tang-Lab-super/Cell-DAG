...
def run(st_data, knn=30):
    ...
    sf = SpaceFlow.SpaceFlow(adata=st_data)
    sf.preprocessing_data(n_top_genes=st_data.shape[1])
    ...
    sf.pseudo_Spatiotemporal_Map(n_neighbors=30, resolution=1.0)
    st_data.obs["ptime"] = sf.pSM_values
...
for sel_batch in all_batch:
    ...
    print(sel_batch)
    ## 获取一个新的
    metadata = all_metadata.loc[all_metadata.Batch == sel_batch].copy()
    expdata = all_expdata.loc[metadata.index].copy()
