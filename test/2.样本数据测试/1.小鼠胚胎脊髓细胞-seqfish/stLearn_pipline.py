...
st_data.obs['imagerow'] = st_data.obs['row']
st_data.obs['imagecol'] = st_data.obs['col']
...
st.pp.neighbors(st_data_use, n_neighbors=25, random_state=0)
...
# sel_cluster = st_data_use.obs.iloc[closest_index]['louvain']

st_data_use.uns["iroot"] = st.spatial.trajectory.set_root(st_data_use, use_label="louvain", cluster='4')
...
plt.subplot(1, 1, 1)
ax = sc.pl.umap(st_data_use, color="dpt_pseudotime", color_map='Spectral_r', title=None)
plt.savefig(f"{save_folder_trajectory}/2.umap_dpt_pseudotime.pdf")

plt.close('all')
fig = plt.figure(figsize=(5, 5))
plt.subplot(1, 1, 1)
sc.pl.embedding(st_data_use, basis="spatial", color="dpt_pseudotime",size=10, s=10, show=False, title='louvain')
plt.savefig(f"{save_folder_trajectory}/3.spatial_dpt_pseudotime.pdf")
